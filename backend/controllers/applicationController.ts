...
export const createApplication = asyncHandler(async (req: CreateAppRequest, res: Response<ApplicationType, ResponseAuthLocals>) => {
  const { jobTitle, companyName, location, link, status, notes } = req.body;
...
export const getAppById = asyncHandler(async (req: Request<{ appId: string }>, res: Response<ApplicationType, ResponseAuthLocals>) => {
...
export const updateApplication = asyncHandler(async (req: Request<{ appId: string }>, res: Response<ApplicationType, ResponseAuthLocals>) => {
  const id = req.params.appId;
  const application = await Application.findById(id);
  // validate app exists
  if (!application) {
...
export const deleteApplication = asyncHandler(
  async (req: Request<{ appId: string }>, res: Response<{ success: boolean; deletedApp: ApplicationType }, ResponseAuthLocals>) => {
    const id = req.params.appId;
    const application = await Application.findById(id);
    // validate app exists
    if (!application) {
